...
//using Type = ExamDotNetMVC.Models.Type;
...
namespace ExamDotNetMVC.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult UserResultChart()
        {
            string query = "SELECT Name, UMarks FROM UserDetails";
            string constr = ConfigurationManager.ConnectionStrings["ExamDotNetMVCContext"].ConnectionString;
            List<UserDetail> chartData = new List<UserDetail>();
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand(query))
                {
                    ...
                    using (SqlDataReader sdr = cmd.ExecuteReader())
                    {
                        while (sdr.Read())
                        {
                            chartData.Add(new UserDetail
                            {
                                Name = sdr["Name"].ToString(),
                                UMarks = Convert.ToInt32(sdr["UMarks"]).ToString()
                            });
                        }
                    }
        ...
        public ActionResult Index1()
        {
            ...
            string Name = Session["DataView"].ToString();
        ...
        public ActionResult ViewResult(string Option, string Name)
        {
            if (Option == "Name")
            {
                return View(db.MarksDetails.Where(w => w.Name.Equals(Name.ToString())).ToList());
            }
        ...
        public ActionResult Details(int? id)
        {
            ...
            var uri = "http://localhost:53349/api/AddQuestions1/" + id;
        ...
        {
            //var items = new AddQuestion();
            //ViewBag.Items = items;
            //var create = new AddQuestion
            //{
            //    Type = new List<Type>
            //    {
            //        new Type {ID = 1, Pattern = "MCQ"},
            //        new Type {ID = 2, Pattern = "BLANK"}
            //    }
            //};
            //ViewBag.Pattern = "MCQ";
            //var type = addQuestion.Type;
            //if (type == ViewBag.Pattern)
            //{
            //    return PartialView("~/Views/WebUsers/Choice", addQuestion);
            //}
            //else
            //{
            //    return PartialView("~/Views/WebUsers/Blank", addQuestion);
            //}
            //return View(create);
            return View();
        }
        ...
        [ValidateInput(false)]
        [ValidateAntiForgeryToken]
        public ActionResult Create([Bind(Include = "Questionid,Question,Type,Answer,QLevel,Marks,OptionA,OptionB,OptionC,OptionD,,UserAnswer,Explaination")] AddQuestion addQuestion, FormCollection formCollection)
        {
            addQuestion.Type= formCollection["Type"];
            //var type = formCollection["Type"];
            //addQuestion.Type = type;
            var uri = "http://localhost:53349/api/AddQuestions1/";
